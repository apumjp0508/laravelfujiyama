...
5. **本番環境でのデプロイがまだ完了していない**
   本番環境でのデプロイがまだ終わっていない具体的には、S3に画像をアップロードできたがその画像をひょうじするのにS3からcloudfrontへの接続が毎回エラーになってしまう、一部のCSSが適用されていないなど、完全な状態では全然ないため、いち早く完全な状態にしたい
